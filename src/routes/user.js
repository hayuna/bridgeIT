...
router
  .route("/:desiredAction((((de)?)activate))/:id")
...
router
  ...
  .put(
    [validateParam(schema.id, "id"), validateBody(schema.user.put)],
    Controller.replace
  )
  .patch(
    [validateParam(schema.id, "id"), validateBody(schema.user.patch)],
    Controller.update
  )
  .delete(validateParam(schema.id, "id"), Controller.deleteUser);
